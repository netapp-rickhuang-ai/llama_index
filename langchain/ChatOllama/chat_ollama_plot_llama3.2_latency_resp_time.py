# [NOTE] must have `ollama serve` running in the background
...
# Stream for lllama3.2: see other examples
llm_0 = ChatOllama(
    model="llama3.2",
    temperature=0.5,
...
if hasattr(ai_msg_0, "response_metadata"):
    ...
    print(f"[OUTPUT] Total Response Duration: {total_duration_ms} ms")
...
labels = ["Total Response", "Model & Data Load", "Prompt Eval Duration"]

plt.figure(figsize=(8, 6))
plt.bar(labels, durations, color=["blue", "orange", "green"])
plt.xlabel("Duration Type")
plt.ylabel("Duration (ms)")
plt.title("Response Latencies in Milliseconds for fixed 'BOXER' prompt")

# Set y-axis limits from 0 to 120,000 ms
plt.ylim(0, 120000)
