...
json_agent_executor = create_json_agent(
    llm=ChatOllama(model="llama3.1", temperature=0.5),  # Updated to use ChatOllama
...
# Create the JSON agent
json_agent_executor = create_json_agent(
    llm=ChatOllama(model="llama3.1", temperature=0.1),  # Updated to use ChatOllama
    toolkit=json_toolkit,
    prompt=final_prompt,
)
...
output = json_agent_executor.invoke(
    {"input": "What is the state of incorporation for Uber Technologies, Inc.?"}
)
print(output)